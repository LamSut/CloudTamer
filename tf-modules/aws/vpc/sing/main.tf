...
resource "aws_eip" "sing_nat_eip" {
  domain = "vpc"
}

resource "aws_nat_gateway" "sing_nat_gw" {
  allocation_id = aws_eip.sing_nat_eip.id
  subnet_id     = aws_subnet.sing_public_subnet_1.id
  depends_on    = [aws_internet_gateway.sing_igw]
}

resource "aws_route_table" "sing_private_rt" {
  vpc_id = aws_vpc.sing_vpc.id
}

resource "aws_route" "sing_private_rt_nat_route" {
  route_table_id         = aws_route_table.sing_private_rt.id
  destination_cidr_block = var.default_cidr
  nat_gateway_id         = aws_nat_gateway.sing_nat_gw.id
}

resource "aws_route_table_association" "sing_private_assoc_1" {
  subnet_id      = aws_subnet.sing_private_subnet_1.id
  route_table_id = aws_route_table.sing_private_rt.id
}

resource "aws_route_table_association" "sing_private_assoc_2" {
  subnet_id      = aws_subnet.sing_private_subnet_2.id
  route_table_id = aws_route_table.sing_private_rt.id
}
